...
#figure(
  ...
  caption: [Таблица Б.1 — Базовая конфигурация APPO],
)
...
#figure(
  ...
  caption: [Таблица Б.2 — Конфигурация Mamba-2, trial #62],
)
...
#figure(
  ...
  caption: [Таблица Б.3 — Конфигурация TransformerCore],
)
...
#figure(
  ...
  caption: [Таблица Б.4 — Конфигурация PerceiverCore],
)
...
#figure(
  ...
  caption: [Таблица Б.5 — Версии программного обеспечения экспериментального стенда],
)

== Потребление видеопамяти (core-only)

В табл. <tab-vram> сведены результаты замеров core-модулей (энкодер и декодер в расчёт не брались). Число параметров и потребление видеопамяти не всегда коррелируют напрямую: Mamba-2 с 0.9M параметров даёт 0.40 GB, а Perceiver IO, у которого параметров больше в 16 раз, — всего 0.42 GB. Всё потому, что SSM-сканирование вынуждено хранить своё внутреннее состояние на каждом шаге, а латентный массив Perceiver IO имеет фиксированный размер 32×512 и не зависит от длины обрабатываемой последовательности. Если же собрать полную модель, 5–9 GB уходит на свёрточный энкодер — он работает с 4096 кадрами и соответствующим объёмом активаций.
...
#figure(
  ...
  caption: [Таблица Б.6 — Потребление видеопамяти core-only (без учёта энкодера и декодера)],
) <tab-vram>
