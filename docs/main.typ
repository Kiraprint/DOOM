...
Для каждой архитектуры выполнялся прогон на 50M шагов. Сначала запускалась sample-factory с фиксированной конфигурацией, чекпоинты сохранялись каждые 10M шагов. Затем выбирался чекпоинт с максимальной средней наградой (best reward) — утилита sample-factory вычисляет среднюю по последним 100 эпизодам. GRU (baseline и оптимизированный) и Mamba-2 повторялись на 3–4 seed; для остальных архитектур, показавших низкую награду, достаточно одного seed, поскольку разброс в таком случае несущественен. Для оценки потолка рекуррентной архитектуры был выполнен дополнительный длительный прогон GRU на 250M шагов.
...
На baseline-конфигурации GRU было сделано три прогона. В лучшем случае средняя награда достигала 14.37, в худшем — 12.43; в среднем по трём запускам вышло 13.58 ± 1.02. Стандартное отклонение на последних итерациях не превышало 0.70. Пропускная способность составляла от 24 до 56 тысяч кадров в секунду — разброс объясняется загрузкой системы в момент замера.

Применение гиперпараметров, найденных HPO для Mamba-2 (AdamW, lr=4.05e-4, weight_decay=0.00196, exploration_loss_coeff=0.002), существенно изменило результаты GRU: seed1 — 17.44, seed2 — 16.00, seed3 — 20.15, средняя — 17.86 ± 2.11. Это на 31% выше baseline; seed3 (20.15) приближается к результату GRU 250M (22.45) при пятикратной экономии вычислительных ресурсов. Стандартные гиперпараметры Sample Factory для doom_benchmark, как оказалось, далеки от оптимальных — настройка важна не меньше, чем выбор архитектуры.

=== Mamba-2: результаты HPO-конфигурации

Для Mamba-2 с конфигурацией trial #62 было выполнено 4 запуска. Средние награды, полученные на разных seed, распределились следующим образом: 14.87, 14.84, 19.10 и 15.97, что дало среднее 16.20 ± 2.01 при максимуме 19.10. Столь высокий разброс (across-seed std 2.01, within-run std 0.61–1.28) связан с природой doom_benchmark — награда сильно флуктуирует от эпизода к эпизоду в зависимости от того, как ведёт себя противник, и к архитектуре это не относится. По сходимости Mamba-2 стабильна, хотя чувствительность к гиперпараметрам остаётся высокой: без HPO качество падает на 20–30%.
...
На рис. @fig-reward видно, что GRU с гиперпараметрами Mamba-2 выходит на 55% финальной награды уже за первые 10M шагов, тогда как Mamba-2 обучается плавнее, без резких скачков. GRU 250M выходит на плато около 150M (рис. @fig-gru-250). Mamba-1, Transformer и Perceiver IO за 50M не поднимаются выше 12-13.

По-видимому, селективному SSM нужно больше данных, чтобы настроить входозависимые параметры — этим объясняется более плавный характер обучения Mamba-2. Разрыв между GRU (17.86) и Mamba-2 (16.20) при этом составляет менее 10% при сопоставимом across-seed std. В порядке убывания средних лучших наград: GRU opt (17.86 ± 2.11), Mamba-2 HPO (16.20 ± 2.01), GRU baseline (13.58 ± 1.02), Mamba-1 (12.59), Perceiver IO (1.97) и Transformer (1.01); отдельно GRU 250M показал 22.45.
