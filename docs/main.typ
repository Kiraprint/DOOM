...
#figure(
  image("gru_cell_mermaid.png", width: 80%),
...
#figure(
  image("self_attention_mermaid.png", width: 80%),
...
#figure(
  image("multihead_attention_mermaid.png", width: 70%),
...
#figure(
  image("mamba2_ssd_algorithm.png", width: 100%),
  caption: [Архитектура селективного SSM-блока Mamba-2 (State Space Dual) из оригинального репозитория @dao2024transformers. Вход проходит через Conv1d и SiLU, затем поступает в селективное SSM, где параметры $B$, $C$, $Delta$ вычисляются от входа.],
) <fig-mamba2>
...
Результаты экспериментов (глава~3) показывают, что Transformer на doom_benchmark достигает лишь 1.01. Mamba-2 — 16.20. Разрыв огромный. Причины — фундаментальные, не в настройках.

Самовнимание считает попарные веса между всеми токенами окна. Для модели порядок не важен — его приходится добавлять через positional encoding. В RL свежие наблюдения объективно важнее старых: кадр, поступивший только что, влияет на решение сильнее, чем кадр 60 шагов назад. Рекуррентные модели (GRU, Mamba-2) имеют это смещение «из коробки». Трансформер — нет.

Вторая проблема — квадратичная сложность. При окне 64 шага она ещё терпима. Но скрытое состояние трансформера нельзя «дополнить»: каждый новый кадр требует пересчёта всей матрицы внимания. Mamba-2 на инференсе просто обновляет состояние — $O(1)$ на шаг.

Третья — требовательность к данным. Трансформеры сходятся при объёмах, которые в RL труднодостижимы. 50M шагов в doom_benchmark — несколько дней счёта. Энкодер ViZDoom выдаёт 512-мерный признак. Для трансформера это мало, attention в таком пространстве плохо дифференцирует позиции.

Четвёртая — память. Трансформер хранит $L times L$ матрицу весов: $64^2 = 4096$ на голову. При 8 головах и 4 слоях — 128K дополнительных элементов на шаг. Mamba-2 хранит conv_state ($4 times 512 = 2048$) и ssm_state ($8 times 128 times 128 = 131072$). С ростом окна разница усугубляется.

Mamba-2 даёт компромисс: качество близко к GRU (16.20 против 17.86), $O(L)$-инференс и константная память. Для визуального RL, где каждый шаг — миллисекунды, это преимущество. Трансформер при окне 64 шага не может развернуть механизм внимания и фактически не обучается.
...
#figure(
  image("perceiver_cross_attn_mermaid.png", width: 85%),
...
#figure(
  image("architecture_diagram_mermaid.png", width: 100%),
...
#figure(
  image("mamba2_state_flow_mermaid.png", width: 95%),
...
При стандартном обучении PyTorch хранит все промежуточные активации каждого слоя для вычисления градиентов на обратном проходе. Для Mamba-2 с 4 слоями, скрытой размерностью 512 и батчем 4096 это даёт значительный расход VRAM: каждый слой хранит входной тензор (4096 x 512), выход после нормы, выход SSM-ядра и т.д.

Gradient checkpointing устраняет эту проблему: промежуточные активации не сохраняются, а пересчитываются заново на обратном проходе из предыдущего слоя. Это классический trade-off: вычисления заменяют память.

#figure(
  image("checkpointing_diagram_mermaid.png", width: 100%),
...
#figure(
  image("sliding_window_mermaid.png", width: 90%),
...
#figure(
  image("hpo_pipeline_mermaid.png", width: 90%),
...
- Batch 4096, rollout 64, recurrence 32, hidden 512
- Наблюдения: ч/б 640x480, FRAME_SKIP=4, стек 4 кадров
- Награда: дельта убийств за шаг (стандартная для doom_benchmark)
- Seed по умолчанию 0. Для seed=42 — аргумент командной строки (в конфигах запусков зафиксировано)
- config.json в train_dir для каждого запуска — для воспроизводимости
- FPS — утилитой sample-factory, среднее за всё обучение
- Метрики в metrics.json каждые 1000 шагов, плюс wandb через callback

Каждый seed запускался в отдельной tmux-сессии. Конфигурация каждого запуска — в train_dir/config.json. Всего около 150 запусков: 85 trials HPO Mamba-2, 15 trials Transformer, 10 trials Perceiver, плюс основной эксперимент с 3-4 seed для ключевых конфигураций. Суммарное GPU-время — около 600 часов.
...
Каждая архитектура — 50M шагов. Этапы:

1. *Запуск.* sample-factory с фиксированной конфигурацией. Чекпоинт каждые 10M шагов.
2. *Оценка.* Выбирается чекпоинт с максимальной средней наградой (best reward). Утилита sample-factory считает среднюю за последние 100 эпизодов.
3. *Повторение.* GRU (baseline и оптимизированный) и Mamba-2 — по 3-4 seed. Остальным — по одному (при низкой награде разброс несущественен).
4. *Длительный прогон.* GRU на 250M шагов — чтобы оценить потолок рекуррентной архитектуры.

== Пилотный скрининг архитектур

Первый этап — быстрая отбраковка. Пять архитектур, 50M шагов, один seed. Результаты — на рис. @fig-reward и в табл. @tab-results.

GRU с гиперпараметрами от Mamba-2 — 20.15. Mamba-2 — 19.10 (лучший seed). Стандартный GRU — 13.58. Mamba-1 отстала — 12.59. Transformer и Perceiver IO неконкурентоспособны: 1.01 и 1.97.

Причины по архитектурам. Трансформеру не хватает окна контекста: 64 токена для ViZDoom — крайне мало. HPO поднял до 2.03, но порядок не изменился. Perceiver IO теряет информацию в кросс-внимании, а его козырь (переменный вход) в ViZDoom не нужен. Mamba-1 уступает Mamba-2 из-за меньшего d_state (16 против 128) и неселективного SSM.
...
GRU с гиперпараметрами Mamba-2 выходит на 55% финальной награды за первые 10M шагов (рис. @fig-reward). Mamba-2 обучается плавнее, но разброс между seed выше. GRU 250M выходит на плато около 150M (рис. @fig-gru-250). Mamba-1, Transformer и Perceiver IO не поднимаются выше 12-13 за 50M.

Характер обучения разный. GRU — резкий рост, потом плато. Mamba-2 — плавно, без скачков. Возможно, селективному SSM нужно больше данных, чтобы настроить входозависимые параметры.

Разрыв между GRU (17.86) и Mamba-2 (16.20) — менее 10%. Across-seed std — 2.01 и 2.11 соответственно. На обе архитектуры влияет стохастичность on-policy RL: сходимость чувствительна к начальному seed.

Ранжирование: GRU opt (17.86 ± 2.11), Mamba-2 HPO (16.20 ± 2.01), GRU baseline (13.58 ± 1.02), Mamba-1 (12.59), Perceiver IO (1.97), Transformer (1.01). GRU 250M (22.45) — за счёт пятикратного увеличения шагов.

GRU устойчив к lr от 1e-4 до 4e-4. Mamba-2 — при отклонении от оптимума награда падает на 20-30%. Perceiver IO чувствителен к размеру латентного массива. Transformer ограничен окном контекста — HPO бесполезна, пока window_size не увеличится.
...
По FPS GRU лидирует. Mamba-2 медленнее из-за Triton-ядер SSM и копирования состояния между очередями, но 15-23K fps достаточно для on-policy. Perceiver IO — 8K, кросс-внимание бутылочное горлышко.

Полная модель (энкодер + core + декодер + буферы) — от 5 до 9 GB. Энкодер ViZDoom занимает основной объём, так что разница между архитектурами не так критична.
...
*Mamba-2* — альтернатива для длинных последовательностей. На 11% ниже GRU при 50M, но $O(L)$ вместо $O(L^2)$. Без HPO падает ниже GRU baseline.
