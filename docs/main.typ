...
Mamba-2 использует два внутренних состояния: conv_state и ssm_state. Они должны сохраняться между шагами эпизода и сбрасываться на его границе. Sample Factory умеет управлять только одним массивом rnn_states, который обнуляется при завершении эпизода — оба состояния Mamba-2 нужно упаковать в один тензор.
...
По гиперпараметрам: GRU устойчив к вариации lr от 1e-4 до 4e-4 — результат меняется незначительно. Mamba-2, напротив, — шаг в сторону от оптимума, и награда падает на 20–30%. Perceiver IO чувствителен к размеру латентного массива (уменьшение с 32 до 16 не дало прироста), а Transformer принципиально ограничен окном контекста — HPO неэффективна, пока window_size не увеличится хотя бы до 256.

= Заключение

В работе рассматривался вопрос выбора архитектуры памяти для RL-агента. Сравнивались GRU, Mamba-2, Transformer и Perceiver IO — для этого на базе ViZDoom и Sample Factory собрали стенд, где модуль памяти подменяется через ModelCore без правки энкодера и декодера. Каждая архитектура потребовала своей реализации: для Mamba-2 пришлось добавить сериализацию conv_state + ssm_state в rnn_states и gradient checkpointing, TransformerCore работает со скользящим окном и каузальной маской, PerceiverCore — с латентным массивом и кросс-вниманием. Гиперпараметры Mamba-2 подбирались байесовской оптимизацией — 85 trials, из которых 54% отвалились по OOM. Для Transformer провели 15 trials. Стандартный прогон — 50M шагов на архитектуру, 3–4 seed для ключевых конфигураций, плюс GRU 250M для оценки потолка.

GRU с гиперпараметрами Mamba-2 (AdamW, lr=4e-4) даёт 17.86 ± 2.11 против 13.58 ± 1.02 у стандартного GRU — прирост +31% только за счёт настройки. Сама Mamba-2 (HPO best) достигает 16.20 ± 2.01, то есть на 11% ниже, но при линейной сложности $O(L)$ против квадратичной у трансформера. GRU 250M вышел на 22.45, после 150M шагов роста нет. Mamba-1 (12.59), Transformer (1.01) и Perceiver IO (1.97) существенно уступают — первые две архитектуры не дают приемлемого качества в данной постановке.
