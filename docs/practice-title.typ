// Custom title page for practice report (otchet po praktike)
// Adapted from vkr-title.typ for the GOST modern-g7-32 template
...
#let arguments(..args, year: auto, city: "Санкт-Петербург") = {
  ...
  args.practice_type = args.at("practice_type", default: none)
...
#let template(
  ...
  practice_type: "производственной практики (научно-исследовательской работы)",
  ...
  if program != none {
    align(left, stack(dir: ttb, spacing: 0.4em,
  ...
  if direction != none {
    align(left, stack(dir: ttb, spacing: 0.4em,
      stack(dir: ltr,
        text(size: 13pt, weight: "bold", "Направление подготовки: "),
  ...
  // === БЛОК 5: Вид практики ===
  align(center, text(size: 14pt, "по " + practice_type))
  v(6pt)
  align(center, text(size: 14pt,
    "Тема: Расширение экспериментальной базы сравнения GRU\n"
    + "и Mamba-2 в on-policy RL: длительные запуски (250M шагов),\n"
    + "вариация контекста (32–128 кадров)\n"
    + "и повышение статистической значимости (20+ seed)"))
  v(150pt)
