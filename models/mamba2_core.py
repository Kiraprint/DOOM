...
class Mamba2Core(ModelCore):
    ...
    def __init__(self, cfg: Config, input_size: int):
        ...
        # Use mamba_num_layers if set (to avoid SF double-counting rnn_num_layers)
        self.num_layers = getattr(cfg, 'mamba_num_layers', cfg.rnn_num_layers)
    ...
    def _forward_with_checkpointing(self, x: torch.Tensor) -> torch.Tensor:
        ...
        def segment_forward(x, residual, norm, mamba_layer):
            x = norm(x)
            x = mamba_layer(x)
            return x + residual
    ...
    def forward(self, head_output, rnn_states):
        ...
        if is_seq:
            # Training: process full sequence with residual connections
            for norm, wrapped in zip(self.mamba_norms, self.mamba_wrapped):
                residual = x_data
                x_data = norm(x_data)
                x_data = wrapped(x_data)
                x_data = x_data + residual  # residual connection
        else:
...
def get_mamba2_required_rnn_size(cfg: Config) -> int:
    ...
    num_layers = getattr(cfg, 'mamba_num_layers', cfg.rnn_num_layers)
