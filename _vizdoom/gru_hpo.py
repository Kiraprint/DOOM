#!/usr/bin/env python
"""
GRU HPO: find best hyperparameters for GRU baseline.

Searches over shared HPs (optimizer, lr, exploration_loss_coeff, weight_decay)
using the unified models.train runner.

Usage:
    python -m gru_hpo --max-trials 15 --time-budget 12
    python -m gru_hpo --dry-run
"""
...
logger = logging.getLogger("gru_hpo")

PROJECT_ROOT = Path(__file__).resolve().parent
sys.path.insert(0, str(PROJECT_ROOT))

TRIAL_TIMEOUT_SECONDS = 2 * 3600  # 2 hours per trial
TRIAL_LIMIT = 15
...
def get_gru_search_space() -> dict:
    """GRU only searches shared HPs. Architecture (rnn_size=512, 1 layer) is fixed."""
    return {
...
def _build_argv(params: dict, trial_id: int, train_dir: str) -> list:
    ...
    argv = [
        ...
        "--experiment", f"gru_hpo_trial_{trial_id}",
    ]
    wd = params.get("weight_decay", 0.0)
    if wd > 0:
        argv.extend(["--weight_decay", str(wd)])
    argv.extend(["--experiment", f"gru_hpo_trial_{trial_id}"])
    return ["uv", "run", "python"] + argv


def _extract_final_reward(log_path: Path) -> float:
    """Parse final Avg episode reward from sf_log.txt."""
...
def run_trial(params: dict, trial_id: int, train_dir: str) -> dict:
    """Run one GRU training trial."""
    ts = time.strftime('%Y%m%d_%H%M%S')
    trial_name = f"gru_hpo_trial_{trial_id}_{ts}"

    argv = _build_argv(params, trial_id, train_dir)
    # Override experiment name with timestamped version
    ...
    try:
        ...
        best_reward = _extract_final_reward(log_file)
...
def objective(trial: optuna.Trial) -> float:
    params = {}
    for pname, spec in get_gru_search_space().items():
...
def run_gru_hpo(max_trials: int = TRIAL_LIMIT, time_budget_hours: float = TIME_BUDGET_HOURS, dry_run: bool = False):
    logger.info("=" * 60)
    logger.info("GRU HPO")
    ...
    if dry_run:
        params = {p: suggest.__code__ for p in get_gru_search_space()}
        logger.info("DRY RUN — config OK: %s", list(get_gru_search_space().keys()))
        return
    ...
    report_path = Path("train_dir") / f"gru_hpo_results_{timestamp}.json"
...
def main():
    parser = argparse.ArgumentParser(description="GRU HPO")
    ...
    run_gru_hpo(max_trials=args.max_trials, time_budget_hours=args.time_budget, dry_run=args.dry_run)
